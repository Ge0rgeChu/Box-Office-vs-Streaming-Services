...
// Colors for each streaming platform
...
// Create a tooltip with the updated "hovering font staff"
...
Promise.all([
  ...
  // Count number of movies for each genre across streaming platforms
  const genreCounts = {};
  ...
  // Sum up box office revenue by genre
  ...
  // Filter to genres that appear in both streaming data and box office data
  const commonGenres = Object.keys(genreCounts).filter(genre => boxOfficeRevenue[genre]);

  // Build final combinedData array
  ...
  // Draw line for box office revenue
  svg.append("path")
      .datum(combinedData)
      .attr("fill", "none")
      .attr("stroke", "red")
  ...
  // Ensure the box office line and dots are on top
